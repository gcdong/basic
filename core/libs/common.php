...
class Common
{
    ...
    {
        $service = CMS::getService('taxonomy');
    ...
    {
        CMS::info($name,__METHOD__.',the form name is '.$name);
    ...
    public static function getAttachUrl($url, $echo = true)
    {
        if($echo)
        {
            echo CMS::getWebUrl() . '/data/attachment/' . $url;
        }
        return CMS::getWebUrl() . '/data/attachment/' . $url;
    }
}
